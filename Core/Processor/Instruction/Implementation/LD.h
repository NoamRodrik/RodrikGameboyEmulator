...
{
	memory.Write(reg_with_addr, reg_with_value);
	return true;
};
...
{
	o_reg = READ_DATA_AT(reg_with_addr);
	return true;
};
...
auto LD_0xF8 = []()
{
	const r8_t right_hand_operand = R8();

	F.MutateByCondition(Tools::CarryOnAddition(SP_const, right_hand_operand), Flag::CARRY);
	F.MutateByCondition(Tools::HalfCarryOnAddition(SP_const, right_hand_operand), Flag::HALF_CARRY);
